#!/usr/bin/env python3
"""
derive_calibrated_2024_csvs.py
Parse a 2024-calibrated MD's tables and emit companion CSVs.
...
Output structure (per AC NN):
  ...
  csv/NN_vote_<axis>_2024.csv         — vote × demographic joints
  csv/NN_calibration_target_2024.csv  — section E aggregate

Run:
    python3 scripts/derive_calibrated_2024_csvs.py 095        # one AC
    python3 scripts/derive_calibrated_2024_csvs.py --all      # every *_2024.md
"""
...
def _ac_2024_dir(ac: str) -> Path:
    """Return constituency_data/constituencies/{ac}_*/2024 for a 3-digit AC id."""
    matches = sorted(CONSTS_DIR.glob(f"{ac}_*"))
    if not matches:
        raise FileNotFoundError(f"No constituency folder for AC {ac!r}")
    return matches[0] / "2024"


# Section blueprint. The structure is identical across all 2024-calibrated ACs;
...
JOINT_FILE_STEMS: dict[str, str] = {
    ...
    "D.15": "vote_religion_2024",
    "D.16": "vote_caste_2024",
    "D.17": "vote_gender_2024",
    "D.18": "vote_welfare_2024",
}
...
def find_md_for_ac(ac: str) -> Path:
    """Find <ac>_*_2024.md inside constituency_data/constituencies/{ac}_*/2024/."""
    ac_2024 = _ac_2024_dir(ac)
    matches = sorted(ac_2024.glob(f"{ac}_*_2024.md"))
    if not matches:
        raise FileNotFoundError(
            f"No MD file matching '{ac}_*_2024.md' in {ac_2024}")
    if len(matches) > 1:
        raise ValueError(f"Multiple MDs match '{ac}_*_2024.md': {matches}")
    return matches[0]


def derive_one(ac: str, verbose: bool = True) -> dict:
    """Parse the AC's MD and write its CSVs. Return a per-AC summary."""
    out_dir = _ac_2024_dir(ac) / "csv"
    ...
    if cal_rows and cal_header:
        cal_out = out_dir / f"{ac}_calibration_target_2024.csv"
...
def discover_acs() -> list[str]:
    """Return all 3-digit AC numbers found as <NNN>_*_2024.md in constituency_data/."""
    out = []
    for p in sorted(CONSTS_DIR.glob("*/2024/*_2024.md")):
...
def main() -> int:
    ...
    p.add_argument("--all", action="store_true",
                    help="Derive for every <NNN>_*_2024.md in calibrated_2024/")
    args = p.parse_args()
