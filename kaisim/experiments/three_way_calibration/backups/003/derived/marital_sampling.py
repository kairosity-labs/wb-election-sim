"""AC 003 (Cooch Behar Uttar) — marital status sampler.

Tuned to hit the AC 003 marital marginal:
  Never_married 27, Currently_married 64, Widowed 8, Separated_divorced 1.

Strategy:
  1. Use married_given_age_gender joint (post-curation it has 5-yr cohort
     keys like '33_37' instead of '33_47').
  2. For non-married personas, allocate to Widowed / Separated / Never
     based on age + gender, calibrated so the residual splits hit target.
"""
...
def sample(rng, ctx, axis, persona, persona_config=None, **kwargs):
    ...
    # Residual: 36% non-married. Target distribution:
    #   Never 27/36 = 75%  Widowed 8/36 = 22%  Separated 1/36 = 3%
    # But conditioned on age:
    ...
    if age_60plus:
        # Female 75% widow; Male 35% widow; rest Never
