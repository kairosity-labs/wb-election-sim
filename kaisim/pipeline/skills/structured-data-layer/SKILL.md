...
2. **Gather sources** in `data/calibrated_<year>/raw/` — Census tables for the AC's CDBs/GPs/Muni, ECI archive for target-year LS result, any sub-AC scrapes available. Apply the **scrape-first rule** (see `memory/feedback_scrape_first.md`): try sub-AC granularity before falling back to district rollup.
...
8. **Update methodology §6 source list** if any new source class was used.
